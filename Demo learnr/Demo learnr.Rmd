...
## Create random tree data based on plot basal area
...
Now let's make a simple graph with this chunk:
...
- The graph is initiated with the function `ggplot()`.
- The variables are called within the aesthetics function `aes()`.
- The geometries are called with `geom_*()` functions. the (*) can be replaced by various type of geometries, 
