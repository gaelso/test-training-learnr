...
set.seed(1)
tree <- tibble(
  dbh          = sample(50:1000, 20, replace = F) / 10,
...
The tree data is stored in a table `tree`. You can visualize the data in a table format by running the table name `tree` in the console.
...
### Generate tree data based on plot basal area

```{r, echo=FALSE}
sliderInput("basal_area", "Basal area (m^2^/ha): ", min = 10, max = 80, value = 40, step = 5)
plotOutput("gr_ba")
```
...
output$gr_ba <- renderPlot({
  ...
  set.seed(1)
  while(ba_round != input$basal_area) {
    vec_dbh <- sample(50:1000, 20) / 10
    ba <- sum(vec_dbh^2 / 10000 * pi)
